...
<body>
	<form method="POST" action="register.php" name="form1" onSubmit="return check()">
		<div class="login" >
			...
			<b><div id="loiemail">Email đã được sử dụng</div></b>
			<div></div>
...
<?php 
	...
	if (isset($_POST['log']) ==TRUE)
	{
		...
		if($id && $ps && $email && $sdt) {
			...
			$checkmail="SELECT email FROM khachhang WHERE email='$email'";
			$result = database::executeQuery($sql);
			$result1 = database::executeQuery($checkmail);
			...
			{
				echo('<script language="javascript">document.getElementById("loiemail").style.display = "block";</script>');
			}
			else {
