<main class=container1 style="background-image:url(IMG/bg.jpg)">
	...
	?>
		...
		?>
			<meta charset="UTF-8">
			<form id="mainForm" method="post" action="xuly_dathang.php" accept-charset="utf-8" onSubmit="return check();">
				<input type="hidden" value="Xác nhận đơn hàng" name="confirm" accept-charset="utf-8" />
				...
				<span>Email:* </span><input type='text' name='email' value='' placeholder="Email" required>
				<div id='checkemail'>*</div>
				<span>Số điện thoại:* </span><input type='text' name='sdt' value='' placeholder="Số điện thoại" required>
				<div id='checksdt'>*</div>
			...
			<table id="giohang">
				...
				?>
							<tr>
								<?php $row = mysqli_fetch_array($result); $tensp=$row['tensp']; ?>
								<td style='padding-left: 50px;'><?php echo $row['tensp']; ?><b> x <?php echo $soluong; ?></b></td>
								<td>
									<p class="thanhtoan"><?php echo $row['gia'] * $soluong; ?></p>
								</td>
			...
			<script type="text/javascript">
				function check() {
					...
					if (!temp.match(pattern)) {
						...
						alert('Định dạng gmail không đúng');
						flag = false;
				...
				function diachi() {
					...
					document.getElementById("checkbox-dia-chi").onchange = function() {
						diachi()
					};
				}
			</script>
